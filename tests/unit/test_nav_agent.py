...
def make_cfg(**agent_overrides) -> types.SimpleNamespace:
    cfg = types.SimpleNamespace(
        ...
        # Must list every exploration field NavAgent reads unconditionally --
        # this fixture mirrors ExplorationConfig by hand, so a field added to
        # the real config and read without a getattr default fails here. (That
        # is the point: it catches the drift. los_visibility_penalty and
        # continuity_weight were missing and broke the two _select_new_frontier
        # tests below.)
...
def make_agent(cfg=None, target="chair", pointnav=None) -> NavAgent:
