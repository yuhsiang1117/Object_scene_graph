...
**损害是方向性的，不是距离**——这是实测而非推测。被 LLM 改掉的目标点，
路径代价与几何选中的几乎一样（5.23 m vs 5.41 m），追逐时长也一样
（40.1 vs 42.2 步），整体轨迹统计基本不动（重选率 41% → 42%）。
agent 走了同样远的路，只是走错了方向。被覆盖掉的其实是**动量项**
（`continuity_weight=2.0`，本项目历史上最大的单项探索收益 +8.5 SR）
和**信息增益**——这两项 ASCENT 本来就没有，所以它无从失去。
9 个变坏的 episode 里有 5 个最后是 explore-fail，从头到尾没提交过任何目标。

**更深一层的原因：我们描述的是 frontier 的错误一侧。**
frontier 是**未知区域的边界**，而我们的区域描述是对场景图做
`objects_near(centroid, 3 m)` 半径查询——场景图里只可能有**已经观测过**的物体，
也就是**已探索**的那一侧。于是 prompt 告诉 LLM 的是「agent 刚刚走过什么」，
而不是「这个边界之外有什么」。ASCENT 没有这个问题：
它的 value map 是沿着相机视锥**朝未观测方向**投影的，
frontier 截图也是**朝向** frontier 的视野。

实测后果（32 次真实决策、91 条描述）：
**选项并集中有 33% 是所有选项共有的**，
31% 的决策里所有选项共享 ≥50% 的物体，选项之间平均 Jaccard 相似度 0.48。
同一个房间边界上相邻的 frontier 会继承同一批已建图家具——
出现最多的描述物体是 `picture, cabinet, desk, shelf, bed, pillow, sofa,
rug, mirror, lamp, door`，全是通用家具，对目标毫无区分度。
这些「决策」有相当一部分其实是披着推理外衣的抛硬币，
而抛硬币必然输给调好的几何 argmax。

所以 Places365 的差距是真的，但不是全部：91 条描述里带房间标签的是 **0%**，
带物体的是 **100%（平均 9.8 个）**。模型不是缺上下文——
它拿到了大量**关于错误区域**的上下文，而且套在一个
「关键字段永远为空」的 few-shot 模板里。
