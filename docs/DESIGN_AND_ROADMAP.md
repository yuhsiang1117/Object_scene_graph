...
1 個房間，(2) LLM 房間標記的 "bedroom" 偏置讓「不同房間拿到不同標籤」
...
**後續追查（2026-07-20）：拆解「卡在 1 房間」的真正成因，發現比
`door_width_m` 更大的瓶頸**——寫 `scripts/room_seg_diagnose.py`
（診斷用，未提交)直接呼叫 `peak_local_max`/`watershed`
（[room_seg.py:30-52](../src/osg/mapping/room_seg.py)）的中間結果，
對 12 個真實 episode 逐一拆解「卡在 1 房間」到底發生在哪一步：

| 成因 | 比例 | 說明 |
|---|---|---|
| **`no_peaks`（連 watershed 種子都沒有)** | **6/12（50%)** | `peak_local_max` 一個局部極大值都找不到，watershed 從未被呼叫，直接 fallback 成整塊 free space |
| `single_watershed_region`（只有 1 個種子) | 2/12（17%) | 找到 1 個峰值，沒有第二個區域可比較合併 |
| `borderline`（多個候選區域，門檻決定合併與否) | 4/12（33%) | 這才是 P1j 前段修的問題 |

**這推翻了本節前段「多半是探索範圍本身就沒跨出一個房間」的猜測**：
`no_peaks` 的 6 個 episode 裡，`ep14/tv_monitor`（34.7m²)、
`ep6/chair`（24.0m²)、`ep2/chair`（23.0m²)、`ep11/bed`（20.3m²）
探索面積都不小，卻連一個距離最近障礙物（含家具，不只牆壁）超過
`min_room_radius_m=0.9m` 的點都找不到——比較像是**家具雜物密度**
的問題：costmap 的 `free_mask()` 把家具也標成 OCCUPIED，房間裡家具
擺得密、走道窄，distance transform 在整個已探索區域裡可能從未超過
0.9m，watershed 連種子都撒不下去，跟探索範圍大小是兩回事。

也確認 `door_width_m=2.0` 這個修正本身很穩：4 個真正走到合併判斷的
episode（`ep7`/`ep0`/`ep3`/`ep13`），在 `door_width_m=2.0` 跟極端值
`6.0` 下房間數完全一樣，沒有卡在門檻邊緣的脆弱性。

**意義**：P1j 前段修的合併門檻只解決了「卡在 1 房間」裡約 1/3 的
case；真正的大宗（過半）成因在更前面一步——`min_room_radius_m` 門檻
跟家具雜物造成的低 clearance 之間的交互作用。這比單純調參數難處理：
直接調小 `min_room_radius_m` 會讓雜亂區域冒出大量假種子（家具縫隙
間的小房間過度分割），不是免費的修法。

**方向 A 與 B（純數值門檻）已測到頭，都無解（2026-07-20）**：
- **方向 A（掃 `min_room_radius_m`）**：0.9→0.25 全範圍掃 12 episode，
  發現是**斷崖不是漸變**——「卡住」的 episode 要降到 0.5~0.6 才找得到
  第二個種子，但「本來就分割正確」的 episode 在 0.75 就開始過度分割
  （ep0 2→7、ep7 3→5），兩群需要的門檻範圍幾乎不重疊，沒有安全值。
- **方向 B（`_drop_small` 改成「小區域併入最大鄰居」而非歸零)**：兩輪
  掃 `merge_min_cells`（60~10000）。修好 ep0 過度分割需要的門檻
  （≥4000）剛好把 ep6/ep3 好不容易解鎖的案例重新壓回 1 房間，互相
  打架。根因：每個 episode「家具縫隙碎片」跟「真正小房間」的格數
  量級逐場景漂移（ep4 家具碎片 1608 格 vs ep7 真實小房間 2097 格，
  重疊），**同一個全域數字門檻物理上不可能同時對所有場景成立**。
  （順帶修掉一個原型 bug：僅剩最後一個區域時不該被丟成 0 房間。）

兩輪合起來的結論：**任何全域數值門檻（距離或格數）都無法乾淨分開
「家具縫隙」和「真正房間」**，因為兩者在該度量下的數值範圍本就重疊
且逐場景漂移。這跟 [3D Scene Graphs survey（arXiv:2606.19383）] 說
「wall-accurate 房間邊界是全領域 open problem」一致。

**方向 C（占據高度分帶，仿 arXiv:2606.13727 Occupancy-Grounded）：
機制可行但實測不改善，不予實作（2026-07-21）**——查了 2025~2026
的 scene graph 論文後，Occupancy-Grounded Room Segmentation 提供一個
不靠全域門檻的思路：用**垂直柱的高度分帶**區分牆與家具（牆從地板頂
到天花板，家具只佔低處），房間分割用「非牆」的 free space，家具被填
回不再切碎房間。先做了低成本原型（`scripts/`，未提交診斷腳本，跑正常
episode 迴圈同時累積每格的雙帶占據計數）：
- **低帶 [0.2,1.0]m**（家具或牆下段) vs **牆帶 [1.3,2.0]m**（高於家具、
  低於天花板,只有真牆會有回波)。修正了本文件稍早那次 max-height
  實驗失敗的原因——天花板 ~2.4m 位於每個室內地板格正上方,用 per-cell
  「最高高度」會把整個地板都標成高（實測 61.8% 格子變紅);改記「牆帶
  內是否有回波」後,天花板被 2.0m 上界排除。
- `room_free = 已探索 ∩ 非牆`。

**機制驗證成功**：牆帶只佔已探索面積 5–19%（是細線非色塊)，room_free
的 `max_dist` 幾乎全部從 <0.9m 升到 >0.9m（ep4 0.55→1.06、ep14
0.78→1.95),**no_peaks 問題確實被解掉了、種子找得到了**。`min_wall_hits`
從 1 到 5 幾乎無差（sim depth 無雜訊)。

**但分割品質在唯一有 ground truth 的 4 個 episode 上全部變差**：

| episode | 已知正確 | 現行 production（door=2.0) | dual-band |
|---|---|---|---|
| ep7 | 3 | **3 ✓** | 2 ✗（丟一間) |
| ep0 | 2 | **2 ✓** | 3 ✗（過度分割) |
| ep3 | 2 | **2 ✓** | 1 ✗（崩併) |
...
視覺化（`outputs/dualband_png/`）顯示根因：牆帶畫出的牆線**斷裂、不
完整**（門口、低牆段、觀測不全處有缺口),缺口讓 watershed 拓撲不可
預測地改變。更根本的張力：這 4 個可驗證 episode 之所以在現行方法下
分割正確,**恰恰因為靠牆家具充當了厚實的房間分隔障礙物**;dual-band
為解 no_peaks 把家具填回,同時移除了這些幫助正確分割的障礙,改依賴
不完整細牆線→全崩。兩個目標（填家具解 no_peaks vs 保留家具當障礙）
在這批資料上直接對立,而 dual-band 落在可測量的錯誤那一側。dual-band
相對現行唯一的「改善」是 ep10/ep2 從 1→2,但**無 ground truth 支持
這兩個該是 2**,很可能跟 ep0 一樣是過度分割。

**最終結論（房間分割這條線）**：`room_door_width_m=2.0`（P1j 前段,
已提交)是唯一經 ground truth 驗證有效且穩定的修正,予以保留作為最終
狀態。真正卡在 1 房間的多為小面積、本就該單一房間的 episode。三個
後續方向（A 距離門檻、B 面積合併、C 高度分帶）都已用真實資料測完:
A/B 證明無全域門檻可用,C 機制可行但實測退步。**不將 C 實作進
production**——會是對唯一 ground truth 的退步。若未來要重啟,關鍵不是
換參數而是**補全牆線**（morphological closing 連接缺口 + 更可靠的
