...
def relink(tracks: List[ObjectTrack], link_dist_m: float = 1.0) -> None:
    ...
    Grouped by (label, floor) so linking never crosses a storey. The 3D
    distance gate usually separates floors on its own -- storeys are ~2.5 m
    apart and link_dist_m is 1.0 -- but not on a mezzanine, and not when an
    ellipsoid centre has drifted; a cross-floor merge then corrupts the
    object's position with the mean of two different objects.
    """
    by_label: Dict[Tuple[str, int], List[ObjectTrack]] = {}
    for tr in tracks:
        if not tr.blacklisted:
            by_label.setdefault((tr.label, getattr(tr, "floor_key", 0)), []).append(tr)
