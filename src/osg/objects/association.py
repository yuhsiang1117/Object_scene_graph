...
class ObjectTrack:
    ...
    # Step until which this track is not offered as a candidate. Distinct from
    # `blacklisted`, which is forever: a VLM rejection made from across a room
    # is evidence about THAT VIEW, not about the object, so it expires and the
    # track can be re-judged from closer. See ObjectLayer.suppress.
    ...
    evidence: float = 0.0
    # Which storey this track lives on (FloorLayer.key -- stable, never
    # renumbered, unlike floor order). 0 for the single-floor agent. Used to
    # keep association and linking within a floor: without it a track one
    # storey down projects into the lower image rows and can steal a detection
    # or merge with a same-category object directly above it.
    floor_key: int = 0
    # Every observation so far was MARGINAL -- hard against a left/right image
    # edge (so the object is mostly out of frame) or at the far end of the depth
    # range. Such a detection is the cheapest kind to make and the most likely
    # to be wrong, and committing to one sends the agent 20 m across the house.
    # Cleared the moment a clean observation arrives; while set, the track is
    # subject to retraction (see ObjectLayer.retract_unconfirmed).
    out_of_range: bool = False
    # Retracted: seen properly from close range and NOT re-detected, so the
    # original sighting is now disbelieved. Distinct from `blacklisted`, which
    # also covers VLM rejections and unreachable targets.
    disabled: bool = False
    # Subsampled world points of the object's surface, accumulated only for the
    # episode's target category (see ObjectLayer.keep_cloud_labels), so memory
    # is bounded by one category however cluttered the scene. Lets the terminal
    # stop measure distance to the object's NEAREST SURFACE rather than to its
    # ellipsoid centre -- HM3D scores distance to a view_point, and view points
    # are tiled around the surface, not the centre.
    points_w: Optional[np.ndarray] = None  # (M, 3)
