...
class Frontier:
    ...
    # Storey this frontier was extracted on (FloorLayer.key). Frontiers from
    # different floors coexist in one selection round once stair frontiers
    # exist, and reaching one means getting to its floor first.
    floor_key: int = 0
    # "explore" (unknown space on this floor) | "stair_up" | "stair_down".
    # Stair frontiers are scored by a prior rather than by info gain, and
    # arriving at one hands over to the agent's CLIMB state.
    kind: str = "explore"
